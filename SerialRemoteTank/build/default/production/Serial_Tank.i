...
void __attribute__((picinterrupt(("")))) RB0_RET() {
    if (INTF && enableRB0)
        ret = 1;
    INTF = 0;
}
...
void main(void) {
    ...
    serial_tx(0x21);
    serial_tx((0x3C & 0xFF00) >> 8);
    serial_tx(0x3C & 0x00FF);

    serial_tx(0x20);
    serial_tx((0x05 & 0xFF00) >> 8);
    serial_tx(0x05 & 0x00FF);
...
void automatico() {
    ...
    if (setEnable == 0) {
        setVolm = (unsigned int) (eeprom_read(1) << 8 | eeprom_read(2));
        setTemp = (unsigned int) eeprom_read(0);
        setAgit = (unsigned int) eeprom_read(3);
    }
    ...
    while (wait) {
        ...
        if (progressao == 1) {
            ...
            if (opcao == 0) {
                ...
                eeprom_write(0, auxTemp);
                unsigned int auxVolm2;
                auxVolm2 = (unsigned int) (((float) auxVolm / (float) 100) * capacidade);
                auxVolm2 &= 0xFF00;
                auxVolm2 = auxVolm2 >> 8;
                eeprom_write(1, auxVolm2);
                auxVolm2 = (unsigned int) (((float) auxVolm / (float) 100) * capacidade);
                auxVolm2 &= 0x00FF;
                eeprom_write(2, auxVolm2);
                eeprom_write(3, auxAgit);
                setEnable = 0;
            }
            return;
        }
